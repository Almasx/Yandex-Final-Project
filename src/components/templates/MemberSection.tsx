...
const MemberSection = ({members = [], loading} : { members? : Author[], loading : boolean}) => {
    ...
    <div className="relative col-span-full bg-gradient-to-r from-white via-black/30 to-white pt-[1px] dark:from-black dark:via-white/30 dark:to-black">
      <section className="flex  flex-col gap-16 bg-light-bg py-16 dark:bg-dark-bg">
          ...
          <div className="flex flex-row justify-between">
            <div className="animate-pulse rounded-xl h-24 w-[20%] bg-light-gray-200 dark:bg-gray-200"></div>
            <div className="animate-pulse rounded-xl h-24 w-[20%] bg-light-gray-200 dark:bg-gray-200"></div>
            <div className="animate-pulse rounded-xl h-24 w-[20%] bg-light-gray-200 dark:bg-gray-200"></div>
            <div className="animate-pulse rounded-xl h-24 w-[20%] bg-light-gray-200 dark:bg-gray-200"></div>
          </div>
        ...
        {loading && <div className="grid grid-cols-4 gap-6"><Mode type="memberCard"></Mode><Mode type="memberCard"></Mode><Mode type="memberCard"></Mode><Mode type="memberCard"></Mode></div>}
        {!loading && <div className="grid grid-cols-4 gap-6">{members.map((member) => <MemberCard key={member.id} {...member} />)}</div>}
      </section>
    </div>
  );
};
